...
start(){
	if [ "$(status)" = "Running" ]; then
		echo -e "[\e[1;31mError\e[0m] Ruby file viewer already running!"
	else
		...
		# attempt #2
		if [ $(command -v ruby >/dev/null 2>&1) ]; then
			echo -e "[\e[1;31mError\e[0m] Unable to find ruby executable. Server \e[1;31mNOT\e[0m started."
		else
...
stop(){
	if [ "$(status)" = "Running" ]; then
		...
		echo -e "[\e[1;31mError\e[0m] Ruby file viewer not running!"
	fi
}
restart(){
	if [ "$(status)" = "Running" ]; then
